...
def frame(original_image, color, percent_of_side):
    ...
    r, g, b = color
    polaroid_mask = PIL.Image.new('RGBA', (width, height), (0, 0, 0, 0))
    drawing_layer = PIL.ImageDraw.Draw(polaroid_mask)
    ...
    drawing_layer.rectangle((0, height, width, height-(thickness*2)), fill=(r, g, b, 255)) # bottom
    ...
    drawing_layer1 = PIL.ImageDraw.Draw(frame_mask)
    
    # Draw four rectangles to fill frame area with alpha=255
    drawing_layer1.rectangle((0, 0, width, thickness), fill=(r, g, b, 255)) #top
    drawing_layer1.rectangle((0, 0, thickness, height), fill=(r, g, b, 255)) # left
    drawing_layer1.rectangle((0, height, width, height-thickness), fill=(r, g, b, 255)) # bottom
    drawing_layer1.rectangle((width, 0, width-thickness, height), fill=(r, g, b, 255)) #right
    ...
    if use_decorative_frame:
        if width == height:
            ...
            result.paste(frame_pic, (0,0), mask=polaroid_mask)
        if width > height:
            frame_pic = PIL.Image.open(os.path.join(os.getcwd(), 'frame2.jpg'))
            frame_pic = frame_pic.resize(result.size)
            result.paste(frame_pic, (0,0), mask=frame_mask)
        if width < height:
            frame_pic = PIL.Image.open(os.path.join(os.getcwd(), 'Portrait Frame.jpg'))
            frame_pic = frame_pic.resize(result.size)
            result.paste(frame_pic, (0,0), mask=frame_mask)
    else:
